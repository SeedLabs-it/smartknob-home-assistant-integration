...
export const getAsyncApps = (hass: HomeAssistant) => {
  return hass.callApi<{ success: any; apps: App[] }>('GET', 'smartknob/apps');
};
...
export const getAsyncAppSlugs = async (hass: HomeAssistant) => {
  const res = await hass.callApi<{ success: any; app_slugs: AppSlug[] }>(
    'GET',
    'smartknob/app_slugs',
  );
...
export const getAsyncKnobs = async (hass: HomeAssistant) => {
  const res = await hass.callApi<{ success: any; knobs: Knob }>(
    'GET',
    'smartknob/knobs',
  );
